...
# 
tmp1 = read_csv(
    filepath_or_buffer=DIR_WORK / 'data' / 'return_simple_last_1d.csv', 
    index_col='open_time', infer_datetime_format=True)
tmp1.index = DatetimeIndex(tmp1.index)
tmp2 = read_csv(
    filepath_or_buffer=DIR_WORK / 'data' / 'srs_return_simple_index_equally_weighted_last_1d.csv', 
    index_col='open_time', infer_datetime_format=True)
tmp2.index = DatetimeIndex(tmp2.index)
tmp2.columns = ['idx_equally_weighted']
tmp3 = read_csv(
    filepath_or_buffer=DIR_WORK / 'data' / 'srs_return_simple_index_volume_weighted_last_1d.csv', 
    index_col='open_time', infer_datetime_format=True)
